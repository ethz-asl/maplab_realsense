...
void ZR300::configureStaticOptions() {
  ...
  zr300_device_->set_option(rs::option::fisheye_gain,
                            realsense_config_.fisheye_gain_);
  zr300_device_->set_option(rs::option::fisheye_exposure,
                            realsense_config_.fisheye_exposure_ms_);
...
void ZR300::enableCameraStreams() {
  zr300_device_->enable_stream(rs::stream::fisheye,
                               realsense_config_.fisheye_width,
                               realsense_config_.fisheye_height,
                               rs::format::raw8, realsense_config_.fisheye_fps);
  zr300_device_->enable_stream(rs::stream::color, realsense_config_.color_width,
                               realsense_config_.color_height, rs::format::rgb8,
                               realsense_config_.color_fps);
}

void ZR300::frameCallback(const rs::frame& frame) {
//  LOG(WARNING) << "frame " << frame.get_stream_type() << " " << frame.get_frame_number();

  rs::stream stream_type = frame.get_stream_type();
  if (stream_type == rs::stream::fisheye) {
    if (frame.get_frame_timestamp_domain() != rs::timestamp_domain::microcontroller) {
      LOG(ERROR) << "Unhappy frame. Skipping.";
      return;
    }

    const double hw_timestamp =
        frame_timestamp_synchronizer_.getTimestampForFrame(frame.get_frame_number());

    sensor_msgs::ImagePtr msg = boost::make_shared<sensor_msgs::Image>();

    CHECK(device_time_translator_->isReadyToTranslate());
    msg->header.stamp = device_time_translator_->translate(hw_timestamp * kMillisecondsToNanoseconds);
...
//  LOG(WARNING) << "Motion " << (rs::event) entry.timestamp_data.source_id << " "
//      << (double) entry.timestamp_data.timestamp << " "
//      << entry.timestamp_data.frame_number;

  if ((rs::event) entry.timestamp_data.source_id == rs::event::event_imu_gyro) {
    ros::Time time_now = ros::Time::now();
    device_time_translator_->update(static_cast<std::uint64_t>(entry.timestamp_data.timestamp * kMillisecondsToNanoseconds), time_now);
    ...
    imu_synchronizer_.registerGyroMeasurement(
        (double) entry.timestamp_data.timestamp, Eigen::Vector3d(entry.axes[0], entry.axes[1], entry.axes[2]), &imu_data);

    for (const auto& item : imu_data) {
      sensor_msgs::ImuPtr msg = boost::make_shared<sensor_msgs::Imu>();

      msg->header.stamp = device_time_translator_->translate(item.timestamp * kMillisecondsToNanoseconds);
  ...
  } else if ((rs::event) entry.timestamp_data.source_id
      == rs::event::event_imu_accel) {
    imu_synchronizer_.registerAccelerometerMeasurement(
        (double) entry.timestamp_data.timestamp, Eigen::Vector3d(entry.axes[0], entry.axes[1], entry.axes[2]));

  } else {
    LOG(FATAL) << "Unknown IMU type.";
  }
}

void ZR300::timestampCallback(const rs::timestamp_data& entry) {
  if ((rs::event)entry.source_id == rs::event::event_imu_motion_cam) {
    frame_timestamp_synchronizer_.registerTimestamp(entry.frame_number, (double)entry.timestamp);
  }
}
